...
# The Android 12+ launch screen (windowSplashScreenAnimatedIcon in
# values-v31/styles.xml). The circle had already been recoloured green when the
# brand changed, but the shape ON the circle was still Telegram's paper plane —
# so the very first thing a user saw on every launch was the old logo. It hid
# here because nothing about the filename says "icon".
#
# The plane used to fly in via a pathData morph. A morph needs both paths to
# have identical node counts, which an arbitrary monogram will not, so the mark
# fades and scales in instead: the circle still springs open, then the P appears
# inside it. Same rhythm, one less thing that can break.
SPLASH_TEMPLATE = """<?xml version="1.0" encoding="utf-8"?>
<!-- Puregram launch screen. Generated by brand/make_icons.py - do not edit by hand. -->
<animated-vector
        ...
        <vector
            ...
            <group
                ...
                <group android:name="bg">
                    <path
                        android:name="circle"
                        android:pathData="%(circle_small)s"
                        android:fillColor="%(green)s"
                        android:fillAlpha="0"
                ...
                <group
                    ...
                    <clip-path
                        android:name="mask"
                        android:pathData="%(circle_full)s"/>
                    <path
                        android:name="mark"
                        android:pathData="%(mark)s"
                        android:fillColor="#ffffff"
                        android:fillAlpha="0"
                        android:strokeWidth="1"/>
                </group>
        ...
        <aapt:attr name="android:animation">
            <set>
                <objectAnimator
                    ...
                    android:valueFrom="%(circle_small)s"
                    android:valueTo="%(circle_full)s"
...
def _splash_circle(radius: float) -> str:
    """A circle centred in the 320 viewport, as four arcs (matches upstream)."""
    cx = cy = 160.0
    k = 0.5523 * radius  # cubic-bezier circle constant
    return (
        "M %.3f %.3f "
        "C %.3f %.3f %.3f %.3f %.3f %.3f "
        "C %.3f %.3f %.3f %.3f %.3f %.3f "
        "C %.3f %.3f %.3f %.3f %.3f %.3f "
        "C %.3f %.3f %.3f %.3f %.3f %.3f Z"
    ) % (
        cx, cy - radius,
...
def write_splash() -> list[Path]:
    """The Android 12+ launch screen, for both the main and standalone modules."""
    # 0.42 of the viewport keeps the P clear of the circle's edge at full scale.
    mark_scale = 320 * 0.42
    xml = SPLASH_TEMPLATE % {
        "circle_small": _splash_circle(50),
        "circle_full": _splash_circle(100),
        "green": BRAND_GREEN_HEX,
        "mark": mark_path(mark_scale, (320 - mark_scale) / 2, decimals=3),
    }
    written: list[Path] = []
    for res in (ANDROID_RES, STANDALONE_RES):
        target = res / "drawable" / "tg_splash_320.xml"
