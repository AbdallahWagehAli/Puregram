...
//            }
    ...
    /** Puregram: a story is openable iff its author's chat could be opened (POLICY_SPEC §3). */
    private boolean puregramStoryAllowed(int account, long dialogId) {
        return org.telegram.messenger.PuregramRules.getInstance().canOpen(account, dialogId);
    }
    ...
    public void open(int account, Context context, TL_stories.StoryItem storyItem, ArrayList<Long> peerIds, int position, StoriesController.StoriesList storiesList, TL_stories.PeerStories userStories, PlaceProvider placeProvider, boolean reversed) {
        ...
        // Puregram: stories obey the same whitelist as chats — refuse to open a
        // non-approved peer's story (self stories are always allowed). Swiping
        // between peers inside the viewer is gated separately in setOnDialogChanged.
        {
            long puregramTarget = 0;
            if (storyItem != null) {
                puregramTarget = storyItem.dialogId;
            } else if (peerIds != null && !peerIds.isEmpty()) {
                int puregramIdx = (position >= 0 && position < peerIds.size()) ? position : 0;
                puregramTarget = peerIds.get(puregramIdx);
            } else if (storiesList != null) {
                puregramTarget = storiesList.dialogId;
            }
            if (puregramTarget != 0 && !puregramStoryAllowed(account, puregramTarget)) {
                org.telegram.messenger.PuregramRules.getInstance()
                        .notifyBlocked(account, puregramTarget);
                doOnAnimationReadyRunnables.clear();
                return;
            }
        }
        ...
        if (containerView == null) {
            ...
            storiesViewPager.setDelegate(new PeerStoriesView.Delegate() {
                ...
                public void onPeerSelected(long dialogId, int position) {
                    // Puregram: block paging into a non-approved peer's story (self
                    // always allowed). The initial tap is gated in open(); this
                    // catches swiping between peers inside the viewer.
                    if (dialogId != 0 && !puregramStoryAllowed(currentAccount, dialogId)) {
                        org.telegram.messenger.PuregramRules.getInstance()
                                .notifyBlocked(currentAccount, dialogId);
                        close(false);
                        return;
                    }
