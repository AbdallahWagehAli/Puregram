...
public class LocaleController {
    ...
    // ── Puregram: device-language-aware brand name ──────────────────────────────
    // The launcher icon label resolves via Android resources (values vs values-ar),
    // but the in-app title and the AppName/Page1Title overrides below must honour
    // the DEVICE system language explicitly — not Telegram's in-app language.

    /** True when the DEVICE system locale is Arabic, regardless of in-app language. */
    ...
    /** Brand name — always the Latin "Puregram", in every device language. */
    public static String getPuregramAppName() {
        return "Puregram";
    }

    /**
     * Puregram: brand override for user-visible strings the cloud/local string pack
     * renders with the "Telegram" wordmark. Returns the branded value, or null to let
     * the normal lookup run. Consulted by BOTH getStringInternal and formatString so
     * cloud-managed / formatted keys (e.g. the settings version footer) are covered too.
     */
    public static String puregramBrandOverride(String key) {
        if (key == null) {
            return null;
        }
        switch (key) {
            case "AppName":
            case "Page1Title":
            case "NotificationHiddenName":
            case "NotificationHiddenChatName":
            case "SecretChatName":
                return getPuregramAppName();
            case "TelegramVersion":
                return "Puregram for Android %1$s";
            case "TelegramFAQ":
            case "TelegramFaq":
                return "FAQ";
            case "TelegramFeatures":
                return "Features";
            case "StorageUsageTelegram":
            case "StorageUsageTelegramLess":
                return "Puregram uses %s of your device storage.";
            case "VoipInCallBranding":
            case "CallViaTelegram":
                return "Puregram Call";
            case "VoipInVideoCallBranding":
            case "VideoCallViaTelegram":
                return "Puregram Video Call";
            case "VoiceCallViaTelegram":
                return "Puregram Voice Call";
            case "VoipInConferenceCallBranding":
                return "Puregram Group Call";
            default:
                return null;
        }
    }
    ...
    private String getStringInternal(String key, String fallback, int fallbackRes, int res) {
        // Puregram: brand override at the ROOT string funnel — beats the cloud
        // string pack (which holds "Telegram" for these keys).
        final String puregramBrand = puregramBrandOverride(key);
        if (puregramBrand != null) {
            return puregramBrand;
        }
    ...
    public static String getString(String key, @StringRes int res) {
        // Puregram: brand override — show the device-language brand name, overriding the
        // cloud/locale string packs (which would otherwise render "Telegram"). This is
        // the single funnel point for every key-based getString lookup.
        if (key != null && (key.equals("AppName") || key.equals("Page1Title"))) {
            return getPuregramAppName();
        }
    ...
    public static String formatString(String key, String fallback, int res, int fallbackRes, Object... args) {
        try {
            // Puregram: brand override — applied here too because formatString reads the
            // cloud string pack directly (e.g. the "TelegramVersion" settings footer).
            String value = puregramBrandOverride(key);
            if (value == null) {
                value = BuildVars.USE_CLOUD_STRINGS ? getInstance().localeValues.get(key) : null;
            }
            if (value == null) {
                if (BuildVars.USE_CLOUD_STRINGS && fallback != null) {
