...
#include <QtCore/QLocale> // Puregram: device-language-aware block dialog
#include <QtGui/QGuiApplication> // Puregram: copy a refused chat's identity
...
void SessionController::showPeerHistory(
		...
		const SectionShow &params,
		MsgId msgId) {
	// Puregram: the account's own rules (ADR-002) decide whether a chat opens.
	// This is the single funnel every open path (list tap, t.me link, @username,
	// search, notification) goes through — the desktop equivalent of the Android
	// ChatActivity gate. peerId == 0 (clearing the view) is allowed; Saved
	// Messages is always open inside canOpen(), same as Android. The refusal box
	// is where a chat normally joins either list, so nothing has to be typed.
	if (peerId
		&& !Puregram::Rules::Instance().canOpen(
			session().data().peer(peerId))) {
		// The retry is what makes "Allow" feel like it did something: without
		// it the rule was written and the user was left on the same screen.
		Puregram::ShowRefusal(
			session().data().peer(peerId),
			crl::guard(this, [=, this] {
				showPeerHistory(peerId, params, msgId);
			}));
		return;
	}
...
void SessionController::openPeerStory(
	...
	// Puregram: stories obey the same rules as chats — the story owner is the
	// peer (own stories are always open). This is the single funnel every
	// story-open path goes through (tray, profile, link, openPeerStories).
	if (!Puregram::Rules::Instance().canOpen(peer)) {
		Puregram::ShowRefusal(peer, crl::guard(this, [=, this] {
