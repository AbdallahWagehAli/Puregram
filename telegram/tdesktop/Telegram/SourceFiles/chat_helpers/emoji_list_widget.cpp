...
#include "puregram/puregram_whitelist.h" // Puregram: discovery-surface flags
...
void EmojiListWidget::sendSearchSetsRequest(const QString &query) {
	if (!Puregram::kStickerSearchEnabled) {
		// Puregram: the public sticker-set index is a discovery surface.
		return;
	}
...
void EmojiListWidget::requestSearchCloud(
	...
	if (!Puregram::kStickerSearchEnabled) {
		// Puregram: searching the public emoji/sticker index is a discovery
		// surface. Installed and recent custom emoji are untouched.
		return;
	}
