...
void SetupSensitiveContent(
	...
	// Puregram: the "show sensitive content" row is hidden — the setting is
	// forced off and the "sensitive" restriction is never ignored
	// (POLICY_SPEC §3, §4).
	return;
...
void OpenFileConfirmationsBox(not_null<Ui::GenericBox*> box) {
	...
	// Puregram: the allow / block lists used to hang off this box. They are a
	// top-level Settings row now (settings_main.cpp) — this box only appears
	// when the user already has custom file extensions, so on a fresh profile
	// the lists could not be reached from anywhere.
