...
#include "puregram/puregram_whitelist.h" // Puregram: the user's own lists
...
void BuildSectionButtons(SectionBuilder &builder) {
	...
	// Puregram: the user's own allow / block lists (POLICY_SPEC §7). Top level,
	// directly above Privacy — the same slot the Android fork uses, and the only
	// place people think to look. It used to sit inside the file-confirmations
	// box, which is itself hidden unless the user had already customised
	// extensions, so on a fresh profile the lists could not be reached at all.
	builder.addButton({
		.id = u"main/puregram_rules"_q,
		.title = Puregram::RulesTitleValue(),
		.icon = { &st::menuIconPermissions },
		.onClick = [] { Puregram::ShowRulesBox(); },
		.keywords = { u"allowed"_q, u"blocked"_q, u"rules"_q, u"puregram"_q },
	});
