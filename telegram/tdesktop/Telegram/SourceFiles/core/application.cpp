...
void Application::run() {
	...
	// Puregram: apply a self-update staged on a previous run (swap the binary +
	// relaunch) before anything else, then quit this old process.
	if (Puregram::ApplyStagedUpdateIfReady()) {
		Quit();
		return;
	}

	// Puregram: apply the cached global policy and keep it fresh (60s poll).
	// Channels and bots stay closed until the first document arrives (ADR-001).
	Puregram::Rules::Instance().start();

	// Puregram: kill Telegram's native updater and poll the Puregram server for a newer
	// Puregram build instead (downloaded next to the exe; applied on the next launch).
	Puregram::StartUpdater();
