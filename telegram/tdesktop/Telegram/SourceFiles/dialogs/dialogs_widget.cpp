...
#include "puregram/puregram_whitelist.h" // Puregram: no global search
...
void Widget::requestPublicPosts(bool fromStart) {
	...
	// Puregram: public-post (hashtag) search is a discovery surface —
	// channels.searchPosts is never sent. An empty result is applied on the
	// next event-loop turn, the way a reply would arrive, after the caller has
	// put the list into its loading state (POLICY_SPEC §4).
	crl::on_main(this, [=] {
...
void Widget::requestMessages(bool fromStart) {
	...
	if (!Puregram::kGlobalSearchEnabled) {
		// Puregram: searching all of Telegram is a discovery surface. The empty
		// result is applied on the next event-loop turn, the way a reply would
		// arrive, so the list leaves its loading state.
		crl::on_main(this, [=] {
			searchApplyEmpty(type, &_searchProcess);
		});
		return;
	}
