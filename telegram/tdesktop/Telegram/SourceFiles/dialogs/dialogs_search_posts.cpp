...
void PostsSearch::setQuery(const QString &query) {
	// Puregram: public-post search is a discovery surface — neither
	// channels.checkSearchPostsFlood nor channels.searchPosts is ever sent;
	// the query is dropped and no state is pushed (POLICY_SPEC §4).
	return;
...
void PostsSearch::requestSearch(const QString &query) {
	...
	if (!Puregram::kGlobalSearchEnabled) {
		// Puregram: public-post search is a discovery surface. Marking the
		// entry loaded settles the list on an empty result.
		entry.loaded = true;
		return;
	}
