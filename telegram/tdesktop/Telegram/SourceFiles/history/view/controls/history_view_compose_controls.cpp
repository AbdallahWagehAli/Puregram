...
void ComposeControls::updateInlineBotQuery() {
	...
	// Puregram: inline bots in the composer are a discovery surface — an
	// "@bot query" is never parsed here, so no bot is resolved and no
	// messages.getInlineBotResults is ever sent (POLICY_SPEC §4).
	const auto query = InlineBotQuery();
