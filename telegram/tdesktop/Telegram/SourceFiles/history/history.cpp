...
#include "puregram/puregram_whitelist.h" // Puregram: blocked chats leave the list
...
bool History::shouldBeInChatList() const {
	// Puregram: a permanently blocked chat leaves the list entirely — it is not
	...
	//
	// This is checked before the pinned branch below on purpose: a pinned chat
	// returns true early, and a pin must not outrank a block. Only blocks hide;
	// an allowed or undecided chat is shown as normal, so the list still tells
	// you what exists.
	if (Puregram::Rules::Instance().blocked(peer)) {
		return false;
	}
