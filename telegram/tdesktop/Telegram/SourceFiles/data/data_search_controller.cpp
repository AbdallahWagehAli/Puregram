...
std::optional<GlobalMediaRequest> PrepareGlobalMediaRequest(
		...
		const QString &query) {
	// Puregram: this is the global media search behind the Links, Media and
	// Files tabs — the exact path that still reached Telegram at large after
	// the obvious search boxes were closed. nullopt is the caller's own
	// "nothing to request" answer, so no UI is left waiting.
