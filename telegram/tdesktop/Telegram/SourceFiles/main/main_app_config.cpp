...
void AppConfig::updateIgnoredRestrictionReasons(std::vector<QString> was) {
	_ignoreRestrictionReasons = get<std::vector<QString>>(
		...
		std::vector<QString>());
	// Puregram: the "sensitive" restriction is never ignored, whatever the
	// account's content settings say (POLICY_SPEC §3).
	_ignoreRestrictionReasons.erase(
		ranges::remove(_ignoreRestrictionReasons, u"sensitive"_q),
		end(_ignoreRestrictionReasons));
