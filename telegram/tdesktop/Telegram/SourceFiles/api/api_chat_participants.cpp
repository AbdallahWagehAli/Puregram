...
void ChatParticipants::loadSimilarPeers(not_null<PeerData*> peer) {
	// Puregram: similar channels / similar bots are a discovery surface —
	// channels.getChannelRecommendations and bots.getBotRecommendations are
	// never sent, so similar() stays empty (POLICY_SPEC §4).
	return;
...
void ChatParticipants::loadRecommendations() {
	// Puregram: channel recommendations are a discovery surface — never
	// requested; the list is reported loaded and empty (POLICY_SPEC §4).
