...
void SensitiveContent::update(bool enabled) {
	// Puregram: sensitive content can never be enabled from this client; the
	// toggle is hidden and the setting is forced off (POLICY_SPEC §1).
	if (enabled || !_canChange.current()) {
		return;
	}
